...
class ExtractHandler:
    ...
    async def extract(
        ...
        self.logger.info(
            f"DEBUG: extraction_response keys: {list(extraction_response.keys())}"
        )
        self.logger.info(f"DEBUG: raw_data_dict: {raw_data_dict}")
        self.logger.info(f"DEBUG: metadata: {metadata}")
        self.logger.info(f"DEBUG: schema: {schema}")
        ...
        if schema and isinstance(
            ...
            try:
                self.logger.info(
                    f"DEBUG: Attempting to validate raw_data_dict: {raw_data_dict}"
                )
                self.logger.info(f"DEBUG: Against schema: {schema}")
                validated_model_instance = schema.model_validate(raw_data_dict)
                self.logger.info(
                    f"DEBUG: Validation successful: {validated_model_instance}"
                )
                ...
                self.logger.info(
                    f"DEBUG: Validation failed, raw_data_dict: {raw_data_dict}"
                )
