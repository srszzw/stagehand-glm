    ...
    async def perform_action(self, action: AgentAction) -> ActionExecutionResult:
        ...
        try:
            ...
            if action_type == "click":
                ...
                self.logger.info(
                    f"点击坐标验证: ({x}, {y}) 在视口 {viewport['width']}x{viewport['height']} 中"
                )
                ...
                if button == "back":
                    ...
                    if not (
                        ...
                        self.logger.warning(
                            f"坐标 ({x}, {y}) 超出视口范围 {viewport['width']}x{viewport['height']}"
                        )
                ...
                # 🧹 可选清空输入框：全选现有内容并删除
                if getattr(specific_action_model, "clear_before", True):  # 默认清空
                    self.logger.info("🧹 清空输入框内容")
                    await self.page.keyboard.press("Control+a")  # 全选
                    await asyncio.sleep(0.05)  # 短暂延迟确保全选完成
                    await self.page.keyboard.press("Delete")  # 删除选中内容
                    await asyncio.sleep(0.05)  # 短暂延迟确保删除完成

                # 输入新内容
                self.logger.info(f"⌨️ 输入文本: '{specific_action_model.text}'")
