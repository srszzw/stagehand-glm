...
def observe(
    ...
    try:
        ...
        logger.info(f"Raw observe response: {content}")
        logger.debug(
            "LLM Response",
            auxiliary={
                "content": content,
        ...
        if isinstance(content, str):
            try:
                ...
                # Handle special box format from some models (like GLM)
                if cleaned_content.startswith(
                    "<|begin_of_box|>"
                ) and cleaned_content.endswith("<|end_of_box|>"):
                    cleaned_content = cleaned_content[16:-14].strip()
...
def extract(
    ...
    **kwargs: Any,
    ...
    # Generate schema information if available
    schema_info = ""
    if schema and hasattr(schema, "model_json_schema"):
        try:
            schema_dict = schema.model_json_schema()
            schema_info = json.dumps(schema_dict, indent=2, ensure_ascii=False)
        except Exception as e:
            logger.warning(f"Failed to generate schema info: {e}")

    user_message = build_extract_user_prompt(instruction, tree_elements, schema_info)
    ...
    try:
        ...
        logger.info(f"Raw extract response: {extract_content}")

        if isinstance(extract_content, str):
            try:
                # Handle markdown code block wrapped JSON
                cleaned_content = extract_content.strip()
                logger.info(f"Before cleaning: {repr(cleaned_content[:200])}")

                if cleaned_content.startswith("```json") and cleaned_content.endswith(
                    "```"
                ):
                    cleaned_content = cleaned_content[7:-3].strip()
                    logger.info("Removed ```json wrapper")
                elif cleaned_content.startswith("```") and cleaned_content.endswith(
                    "```"
                ):
                    cleaned_content = cleaned_content[3:-3].strip()
                ...
                if cleaned_content.startswith(
                    ...
                    cleaned_content = cleaned_content[
                        16:-14
                    ].strip()  # Remove <|begin_of_box|> and <|end_of_box|>
                    logger.info("Removed <|begin_of_box|> wrapper")

                logger.info(f"After cleaning: {repr(cleaned_content[:200])}")
                extracted_data = json.loads(cleaned_content)
                logger.info(f"Parsed extracted_data: {extracted_data}")
                ...
                # If JSON parsing fails, try to guide the LLM to provide better output
                logger.warning(
                    "LLM returned non-JSON response. This indicates the prompt needs improvement."
                )
                logger.info(
                    "Consider improving the system prompt or using more specific instructions."
                )
                extracted_data = {}
        else:
            extracted_data = extract_content
            logger.info(f"Non-string extract_content: {extract_content}")
    except Exception as e:
    ...
    try:
        ...
        logger.info(f"Raw metadata response: {metadata_content}")

        # Parse metadata content
        if isinstance(metadata_content, str):
            try:
                # Handle markdown code block wrapped JSON
                cleaned_content = metadata_content.strip()
                if cleaned_content.startswith("```json") and cleaned_content.endswith(
                    "```"
                ):
                    cleaned_content = cleaned_content[7:-3].strip()
